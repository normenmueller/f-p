...
import fp.model.{SiloSystemId, Response, MsgId}
...
trait MessagingLayer[Context] {
  ...
  type MessageHub = Mmap[SiloSystemId, MessagingStatus]
  ...
  def initMessagingHub: MessageHub =
    Mmap.empty[SiloSystemId, MessagingStatus]
  ...
  def unconfirmedResponses: Mmap[SiloSystemId, SelfDescribing]
